...
class AttributeFilter:
    ...
    def __call__(self, approach):
        ...
        return self.op(self.get(approach), self.value)  # returns boolean
...
def create_filters(date=None, start_date=None, end_date=None,
    ...
    if start_date:
        filters_collection.append(DateFilter(operator.ge, start_date))
    if end_date:
        filters_collection.append(DateFilter(operator.le, end_date))
    if distance_min:
        filters_collection.append(DistanceFilter(operator.ge, distance_min))
    if distance_max:
        filters_collection.append(DistanceFilter(operator.le, distance_max))
    if velocity_min:
        filters_collection.append(VelocityFilter(operator.ge, velocity_min))
    if velocity_max:
        filters_collection.append(VelocityFilter(operator.le, velocity_max))
    if diameter_min:
    ...
    if hazardous is not None:
        filters_collection.append(HazardousFilter(operator.eq,
                                                  bool(hazardous)))
